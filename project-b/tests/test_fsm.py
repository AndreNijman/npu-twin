...
def test_unknown_to_present_on_first_face():
    f = PresenceFSM(grace_period_s=30.0, now=0.0)
    assert f.state == State.UNKNOWN
    tr = f.observe(True, now=1.0)
    assert tr is not None and tr.frm == State.UNKNOWN and tr.to == State.PRESENT


def test_present_stays_present():
    f = PresenceFSM(grace_period_s=30.0, now=0.0)
    f.observe(True, now=0.5)
...
def test_unknown_without_face_enters_grace():
    f = PresenceFSM(grace_period_s=30.0, now=0.0)
    tr = f.observe(False, now=1.0)
    assert tr is not None and tr.frm == State.UNKNOWN and tr.to == State.AWAY_GRACE
